...
def main():
    ...
    # of the component
    logging.debug(f"action: config | result: success | logging_level: {logging_level} | input_queues: {input_queues} | output_exchange: {output_exchange} | cluster_size: {cluster_size} | id: {id} | control_queue: {control_queue}")

    movies_joiner = MoviesJoiner(input_queues, output_exchange, cluster_size, id, control_queue)
    movies_joiner.run()
